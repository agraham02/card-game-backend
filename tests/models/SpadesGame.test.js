...
const SpadesGame_1 = require("../../models/game/SpadesGame");
...
describe("Spades Game Mechanics", () => {
    ...
    beforeEach(() => {
        ...
        spadesGame = new SpadesGame_1.SpadesGame(room);
...
describe("SpadesGame - startGame", () => {
    ...
    test("should initialize the game correctly with 4 players", () => {
        const spadesGame = new SpadesGame_1.SpadesGame(room);
        spadesGame.startGame();
...
describe("SpadesGame - handlePlayerAction", () => {
    ...
    test("should handle a valid card play", () => {
        ...
        const playerHand = spadesGame.getGameStateForPlayer(currentPlayerId).hand;
        ...
        expect(spadesGame.getGameStateForPlayer(currentPlayerId).hand).not.toContainEqual(cardToPlay);
        // Verify that the game state is updated (e.g., current turn changes)
        expect(spadesGame.gameState.currentTurnIndex).not.toBe(currentTurnIndex);
    });
        ...
        while (!gameOver) {
            ...
            for (let i = 0; i < 13; i++) {
                for (let j = 0; j < 4; j++) {
                    const currentPlayerIndex = spadesGame.gameState.currentTurnIndex;
                    const currentPlayerId = spadesGame.turnOrder[currentPlayerIndex];
                    const playerHand = players.find((p) => p.id === currentPlayerId).gameData["spades"].hand;
